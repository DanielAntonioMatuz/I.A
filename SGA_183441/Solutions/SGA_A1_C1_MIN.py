...
Genome = 5  # Valor de bits que tendra cada individuo (cromosomas)
...
popSize = N + 1
gen_tamanio = (Genome * numberVariablesEc) + 1
...
cromosoma_poblacion = np.ones([popSize, gen_tamanio], dtype=np.int)
n_cromosoma_poblacion = np.ones([popSize, gen_tamanio], dtype=np.int)
poblacionAnterior = np.ones([popSize, gen_tamanio], dtype=np.int)

H1 = np.ones([popSize, gen_tamanio], dtype=np.int)
H2 = np.ones([popSize, gen_tamanio], dtype=np.int)

mejor_individuo_pop = np.ones([generaciones], dtype=np.int)
mejor_gen_individuo = 0
generation = 0
GraphicAUX = []
...
def calcular_estado_generacion():
    fitness_total = 0
    parent = 0
    for i in range(1, popSize):
        fitness_total = fitness_total + fitness[i]
...
def Proceso_mutacion(pop_mutation_rate, mutation_rate, generation):
    varAuxMut = 0
    for i in range(1, popSize):
...
def Proceso_cruza(Pc):
    ...
    if random.random() <= Pc:
        punto_cruza = np.random.random_integers(gen_tamanio - 2)
    j = 1
...
def Graficar():
    ...
    plt.plot(x, sorted(y, reverse=True), color="red", label="Fitness MINIMIZAR")
...
def IniciarSGA():
    ...
    while (generation < generaciones - 1):
        ...
        print()
        calcular_estado_generacion()
        generation = generation + 1
        Cruza(0.75)
        Proceso_mutacion(0.1, 0.2, generation)
